...
BIBFILE := $(shell perl -ne '($$_)=/^[^%]*\\bibliography\{(.*?)\}/;@_=split /,/;foreach $$b (@_) {print "$$b "}' $(SRC))
...
$(TRG)	: %.pdf : %.tex $(DEP) $(PDFPICS) $(BIBFILE)
	  @$(run-latex)
	  open memoria_portegam.pdf

clean	:
	  -rm -f $(TRG) $(PSF) $(PDF) $(TRG:%.pdf=%.aux) $(TRG:%.pdf=%.bbl) $(TRG:%.pdf=%.blg) $(TRG:%.pdf=%.log) $(TRG:%.pdf=%.out) $(TRG:%.pdf=%.lof) $(TRG:%.pdf=%.lot) $(TRG:%.pdf=%.run.xml) $(TRG:%.pdf=%.toc)

kpdf	:
	  -rm -f *.aux *.blg *.lof *.log *.lot *.out *.run.xml *.toc *.bbl
